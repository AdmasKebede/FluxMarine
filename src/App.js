...
export default class App extends Component {
  ...
  initPlaceApiautocomepeteto() {
    ...
    this.autocompleteTo.addListener("place_changed", () => {
      ...
      this.setState({ to: place.names, destPlaceDetails: place });
    });
  }
  ...
  handleSubmit(event) {
    ...
    if (sourcePlaceDetails.geometry && destPlaceDetails.geometry) {
      axios
        .get(
          `https://api.searoutes.com/route/v2/sea/${sourcePlaceDetails.geometry.location.lng()},${sourcePlaceDetails.geometry.location.lat()};${destPlaceDetails.geometry.location.lng()},${destPlaceDetails.geometry.location.lat()}?avoidHRA=false&avoidSeca=false&allowIceAreas=true&vesselDraft=${+this
            .state.waterDepth}`,
          {
            headers: {
        ...
        .then((res) => {
          if (res.data.features) {
            this.setState({
              Distance: res.data.features[0].properties.distance,
            });
        ...
        <div className="page">
            ...
            {this.state.openForm === true ? (
              <div class="form">
                ...
                <form onSubmit={this.handleSubmit}>
                  ...
                  <input
                    type="text"
                    placeholder="Water depth (meters)"
                    name="waterDepth"
              ...
              <img
                alt=""
                src={require("../src/assets/images/abc.png")}
                width="40px"
                height="40px"
          ...
          {this.state.haveResponse === true ? (
            <div className="right-icon">
              <h4>
                Total Distance{" "}
                <span>{this.getMiles(this.state.Distance)} Miles</span>
              </h4>
              <h4>
                Total Time{" "}
                <span>{this.formateCalculatedTime(this.state.time)}</span>
              </h4>
              <h4>
                Wind <span>{this.state.wind} knots</span>
              </h4>
            </div>
          ) : null}
